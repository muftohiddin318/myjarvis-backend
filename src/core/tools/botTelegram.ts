...
const validate = (args: unknown): ToolValidationResult => {
  ...
  if (target !== "owner" && typeof target !== "string" && typeof target !== "number") {
    return { ok: false, error: "invalid_chat_id" };
  }
  return {
    ok: true,
    value: { chatId: String(target), message: obj.message } satisfies BotSendArgs
  };
};
...
function resolveChatId(chatId: string) {
  ...
  if (ownerUsername && chatId.replace(/^@/, "").toLowerCase() === ownerUsername) {
    if (!ownerId) throw new Error("telegram_bot_owner_not_configured");
    return ownerId;
  }
  ...
  throw new Error("telegram_bot_usernames_cannot_be_resolved_for_direct_messages_use_owner_or_numeric_chat_id");
}

export const botTelegramTools: ToolDefinition<BotSendArgs>[] = [{
  ...
  inputSchema: {
    ...
    required: ["message"],
    properties: {
      chatId: { type: "string", description: "Use 'owner' for the configured MyJarvis owner chat or a numeric Telegram chat ID." },
  ...
  execute: async (args) => {
    ...
    return { sent: true, channel: "telegram_bot", chatId, confirmed: true, telegramResult };
  }
}];
