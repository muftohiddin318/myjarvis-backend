...
const validateSend = (args: unknown): ToolValidationResult => {
  ...
  return {
    ok: true,
    value: { username: obj.username.trim(), message: obj.message } satisfies TelegramSendArgs
  };
};

export const telegramTools: ToolDefinition<TelegramSendArgs>[] = [{
  ...
  inputSchema: {
    ...
    properties: {
      username: { type: "string", description: "Telegram username, with or without the @ prefix." },
