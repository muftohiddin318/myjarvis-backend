...
export async function executeTool(
  name: string,
  args: unknown,
  allowedRisk: keyof typeof APPROVAL_RANK = "read"
  ...
  try {
    ...
    if (validation && !validation.ok) {
      return { ok: false, tool: name, error: validation.error };
    }

    const safeArgs = validation?.ok ? validation.value : args;
    const result = await tool.execute(safeArgs as never);
